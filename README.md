...
```
database/migrations  # Contains required database migrations
...
## Environment
```
Server will load dependencies based on .env file, you use the env.example or custom by yourself.
...
the payout schedule will be calculate based on those values
example :
PAYOUT_INTERVAL = 3
